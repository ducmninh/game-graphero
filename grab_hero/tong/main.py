...
class Game:
    def __init__(self):
        ...
        self.menu_bg = None
        try:
            bg_path_png = SPRITES / "menu_bg.png"
            bg_path_jpg = SPRITES / "menu_bg.jpg"
            if bg_path_png.exists():
                img = pygame.image.load(str(bg_path_png)).convert()
                self.menu_bg = pygame.transform.scale(img, (SCREEN_WIDTH, SCREEN_HEIGHT))
            elif bg_path_jpg.exists():
                img = pygame.image.load(str(bg_path_jpg)).convert()
                self.menu_bg = pygame.transform.scale(img, (SCREEN_WIDTH, SCREEN_HEIGHT))
        except:
            pass
        self.running = True
    ...
    def load_level(self, idx):
        ...
        # Nạp đầy đạn cho tất cả súng mỗi khi bắt đầu màn chơi
        for w_key, w in self.player.weapons.items():
            w.reloading = False
            w.ammo_in_mag = w.mag_size
            w.ammo_reserve = w.spec.get("ammo_reserve", 9999)
            ...
            if self.scene == SCENE_MENU:
                ...
                if event.type == pygame.KEYDOWN:
                    if event.key in (pygame.K_UP, pygame.K_w):
                        self.menu_row = (self.menu_row - 1) % 7
                        ...
                        if self.menu_row == 0: # Journey
                            ...
                            self.scene = SCENE_BOSS_RUSH_MENU
                            self.boss_rush_sel = 0
                        elif self.menu_row == 4: # Shop
                            self.open_hub()
                            if self.hub: self.hub.tab = 0
                        elif self.menu_row == 5: # Character Select
                            self.scene = SCENE_CHAR_SELECT
                            self.char_sel_idx = 0 if self.char_type == "grab" else 1
                        elif self.menu_row == 6: # Exit
                            self.running = False
    ...
    def draw_play(self):
        ...
        for p_id, data in self.network.peer_data.items():
            ...
            if view_rect.collidepoint(pos.x, pos.y):
                ...
                is_shooting = data.get("is_shooting", False)
                if is_shooting:
                    p_obj.weapon.shoot_anim = 0.1
                    p_obj.weapon.ammo_in_mag = p_obj.weapon.mag_size  # infinite ammo for dummies
                    if p_obj.weapon.can_fire():
                        muzzle = p_obj.pos + Vec(math.cos(p_obj.aim_angle), math.sin(p_obj.aim_angle)) * 26
                        new_bullets = p_obj.weapon.fire(muzzle, p_obj.aim_angle, owner=str(p_id))
                        if new_bullets:
                            self.bullets.extend(new_bullets)
                            self.play_sound(p_obj.weapon.key)
