...
def _find_free_spawn_pos(world, x_min, x_max, y_min, y_max, radius=22, max_attempts=100):
    for _ in range(max_attempts):
        x = random.randint(int(x_min), int(x_max))
        y = random.randint(int(y_min), int(y_max))
        test_rect = pygame.Rect(x - radius, y - radius, radius * 2, radius * 2)
        if not world.collides(test_rect):
            return Vec(x, y)
    return Vec((x_min + x_max) // 2, (y_min + y_max) // 2)
...
def build_level2(world=None):
    ...
    # zombies in groups
    for _ in range(10):
        pos = _find_free_spawn_pos(world, 6 * TILE, 40 * TILE, 18 * TILE, 28 * TILE)
        kind = "zombie_fast" if random.random() < 0.6 else "zombie"
        enemies.append(Enemy(kind, pos))
    # thieves with pistols mid-map
    for _ in range(6):
        pos = _find_free_spawn_pos(world, 28 * TILE, 60 * TILE, 18 * TILE, 28 * TILE)
        enemies.append(Enemy("thief_pistol", pos))
    # wild dogs around alleys
    for _ in range(5):
        y_range = random.choice([(2 * TILE, 18 * TILE), (28 * TILE, 44 * TILE)])
        pos = _find_free_spawn_pos(world, 28 * TILE, 70 * TILE, y_range[0], y_range[1])
        enemies.append(Enemy("wild_dog", pos))
    # more thieves near boss area
    for _ in range(5):
        pos = _find_free_spawn_pos(world, 72 * TILE, 92 * TILE, 18 * TILE, 28 * TILE)
        enemies.append(Enemy("thief_pistol", pos))
...
def build_level3(world=None):
    ...
    for _ in range(8):
        pos = _find_free_spawn_pos(world, 18 * TILE, 60 * TILE, 20 * TILE, 36 * TILE)
        enemies.append(Enemy("minion_smg", pos))
    # snipers on raised positions (right side)
    for _ in range(4):
        y_val = random.choice([10 * TILE, 42 * TILE])
        pos = _find_free_spawn_pos(world, 60 * TILE, 88 * TILE, y_val, y_val)
        enemies.append(Enemy("sniper", pos))
    # wild dogs roaming
    for _ in range(6):
        pos = _find_free_spawn_pos(world, 20 * TILE, 80 * TILE, 20 * TILE, 38 * TILE)
        enemies.append(Enemy("wild_dog", pos))
    # extra mid-range pistol thieves
    for _ in range(6):
        pos = _find_free_spawn_pos(world, 24 * TILE, 80 * TILE, 20 * TILE, 36 * TILE)
        enemies.append(Enemy("thief_pistol", pos))
...
def build_level4(world=None):
    ...
    for _ in range(12):
        pos = _find_free_spawn_pos(world, 15 * TILE, 80 * TILE, 10 * TILE, 55 * TILE)
        enemies.append(Enemy("minion_smg", pos))
    for _ in range(6):
        pos = _find_free_spawn_pos(world, 40 * TILE, 85 * TILE, 10 * TILE, 55 * TILE)
        enemies.append(Enemy("sniper", pos))
...
def build_level7(world=None):
    ...
    # Abundant medkits and ammo for the final boss fight
    for _ in range(30):
        # Scatter across the whole map
        px = random.randint(200, 2000)
        py = random.randint(200, 2000)
        world.add_pickup(Pickup(Vec(px, py), random.choice([P_MEDKIT, P_AMMO, P_MEDKIT])))
        
    for _ in range(20):
        # Scatter specifically around the boss arena
        px = random.randint(2200, 2800)
        py = random.randint(600, 1400)
        world.add_pickup(Pickup(Vec(px, py), random.choice([P_MEDKIT, P_MEDKIT, P_AMMO])))
...
def build_pvp_arena(world=None):
    ...
    for _ in range(100):
        ...
        kind = random.choice([P_MEDKIT, P_AMMO, P_AMMO, P_AMMO]) # 75% chance for ammo, 25% for medkit
